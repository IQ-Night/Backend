...
module.exports = (io) => {
  ...
  function startTimer({ roomId, player, list }) {
    ...
    io.to(roomId).emit("JustifyTimer2Update", timeLeft); // Emit the initial time left
    ...
    timers[roomId] = setInterval(async () => {
      ...
      io.to(roomId).emit("JustifyTimer2Update", timeLeft); // Emit the updated time left
      ...
      if (timeLeft <= 0) {
        ...
        io.to(roomId).emit("JustifyTimer2End", {
          player,
          list,
          players,
        });
