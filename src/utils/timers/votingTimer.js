...
module.exports = (io) => {
  ...
  function startTimer({ roomId }) {
    ...
    io.to(roomId).emit("VotingTimerUpdate", timeLeft); // Emit the initial time left
    ...
    timers[roomId] = setInterval(async () => {
      ...
      io.to(roomId).emit("VotingTimerUpdate", timeLeft); // Emit the updated time left
      ...
      if (timeLeft <= 0) {
        ...
        const lastVotes = currentDay.lastVotes;
        ...
        io.to(roomId).emit("VotingTimerEnd", {
          lastVotes: result,
